...
export async function GET(context: APIContext) {
  // Get all Arabic blog posts
  const arabicBlogEntries = await getCollection("blog", ({ id }) => {
    return id.startsWith("ar/");
  });

  // Sort by publication date (newest first)
  const sortedBlogPosts = arabicBlogEntries.sort(
  ...
  return rss({
    title: `${SITE.title} - أحدث المقالات`,
    description: "ابق على اطلاع بأحدث الاتجاهات والتطورات في التكنولوجيا اللامركزية مع رؤى من فريق خبراء أنجور.",
    site: context.site ?? SITE.url,
    items: sortedBlogPosts.map((post) => ({
      title: post.data.title,
      pubDate: post.data.pubDate,
      description: post.data.description,
      author: `${post.data.author} (${post.data.role || 'الكاتب'})`,
